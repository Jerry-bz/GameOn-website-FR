...
// Fonction de validation pour le prénom
 function validForname() {
  if (forms.forename.value.length <2) {
    formData[0].appendChild(p).innerHTML ="Veuillez entrer 2 caractères ou plus pour le champ du prénom.";
    return false;
  } else if (!forms.forename.value.match(nameRegex)) {
    formData[0].appendChild(p).innerHTML ="Veuillez insérer des caractères valide";
    return false;
  } else {
...
function validSurname() {
  if (forms.surname.value.length <2) {
    formData[1].appendChild(p).innerHTML ="Veuillez entrer 2 caractères ou plus pour le champ du nom.";
    return false;
  } else if (!forms.surname.value.match(nameRegex)) {
    formData[1].appendChild(p).innerHTML = "Veuillez insérer des caractères valide";
    return false;
  } else {
...
function validEmail() {
  if (!forms.email.value.match(emailRegex)) {
    formData[2].appendChild(p).innerHTML ="Veuillez insérer une adresse mail valide";
    return false;
  } else {
...
function validQuantity() {
  if (forms.quantity.value.length == 0 ) {
    ...
    formData[4].appendChild(p).innerHTML = "Veuillez insérer un nombre compris entre 0 et 99";
    return false;
  }
  else {
...
function validConditions() {
  let conditionsError = false
    let conditions = document.getElementById("checkbox1")
      ...
      if (!conditionsError) {
        forms.conditions.appendChild(p).innerHTML = "Vous devez vérifier que vous acceptez les termes et conditions";
        return false;
      }
};
